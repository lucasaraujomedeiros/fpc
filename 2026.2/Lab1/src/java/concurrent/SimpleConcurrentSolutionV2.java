...
public class SimpleConcurrentSolutionV2 {
    ...
    private static class ResourceCheckTask implements Runnable {
        ...
        public void run() {
            ...
            for (int i = 1; i <= 5; i++) {
                CheckOneResource checkOneResource = new CheckOneResource(random, i);
                Thread tCheckOneResource = new Thread(checkOneResource, "checa um recurso");
                tCheckOneResource.start();
            }
    ...
    private static class CheckOneResource implements Runnable {

        private Random random;
        private int number;

        public CheckOneResource(Random random, int number) {
            super();
            this.random = random;
            this.number = number;
        }
        ...
        public void run() {
            ...
            try {
                    ...
                    System.out.println("[" + currentThread.getName() + "] Verificando Recurso " + number + " (Duração: " + sleepTime + "ms)...");
